(function( $ ){
  $.fn.downloadPage = function(options) {
    var pluginOptions = {
      attr : "href",
      url : false,
      showMessage: true,
    ...
    this.on("click", function(){  loadDownloadDocument(this, pluginOptions); return false;  });
    
   function loadDownloadDocument(el, pluginOptions){
    ...
    function addIframeToPage(el, pluginOptions){
        ...
        if(!$('#downloadPage')[0]){
          ...
          $('#downloadPage').on("load",function() {  downloadit(pluginOptions);  })
        }else{
          $('#downloadPage').attr("src", url);
        }
    }
    function downloadit(){
      frames["downloadPage"].focus();
      if(pluginOptions.showMessage){
        unloadMessage();
      }
    }
    ...
    var components = {
      iframe: function(url){
        return '<iframe id="downloadPage" name="downloadPage" src='+url+' style="display: none; @media print { display: block; }"></iframe>';
      },
